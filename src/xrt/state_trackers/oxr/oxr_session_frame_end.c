...
static bool
is_blend_mode_supported_by_system_compositor(struct oxr_session *sess, enum xrt_blend_mode blend_mode)
{
	...
	const struct xrt_system_compositor_info *sys_info = &xsysc->info;
	for (uint8_t i = 0; i < sys_info->supported_blend_mode_count; i++) {
		if (sys_info->supported_blend_modes[i] == blend_mode) {
			return true;
		}
	}
...
oxr_session_frame_end(struct oxr_logger *log, struct oxr_session *sess, const XrFrameEndInfo *frameEndInfo)
{
	...
	if (!is_blend_mode_supported_by_system_compositor(sess, blend_mode)) {
