...
enum xrt_device_feature_type
{
	...
	XRT_DEVICE_FEATURE_CAMERA_PASSTHROUGH,
	XRT_DEVICE_FEATURE_MAX_ENUM,
};

/*!
 * A single camera passthrough frame that can be imported by a compositor.
 */
struct xrt_passthrough_camera_frame
{
...
struct xrt_device
{
	...
	/*!
	 * @brief Get the latest camera frame for passthrough composition.
	 *
	 * The returned handle must have one reference held for the caller.
	 */
	xrt_result_t (*get_passthrough_camera_frame)(struct xrt_device *xdev,
	                                             struct xrt_passthrough_camera_frame *out_frame);
...
/*!
 * Helper function for @ref xrt_device::get_passthrough_camera_frame.
 *
 * @copydoc xrt_device::get_passthrough_camera_frame
 *
 * @public @memberof xrt_device
 */
XRT_NONNULL_ALL static inline xrt_result_t
xrt_device_get_passthrough_camera_frame(struct xrt_device *xdev, struct xrt_passthrough_camera_frame *out_frame)
{
	if (xdev->get_passthrough_camera_frame == NULL) {
		return XRT_ERROR_FEATURE_NOT_SUPPORTED;
	}
	return xdev->get_passthrough_camera_frame(xdev, out_frame);
}
