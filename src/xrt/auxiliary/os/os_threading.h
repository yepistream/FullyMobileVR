...
/*
 * Used in the 6DoF modified android driver.
 *      Reseaon : Hopefully to fix some timestamp problems with android phone os.
 * */
static inline int
os_thread_helper_init_monotonic(struct os_thread_helper *oth)
{
    U_ZERO(oth);
    ...
    ret = pthread_cond_init(&oth->cond, &attr);
