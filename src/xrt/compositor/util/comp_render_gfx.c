...
static const VkClearColorValue background_color_transparent = {
...
crg_clear_output(struct render_gfx *render, const struct comp_render_dispatch_data *d)
{
	const VkClearColorValue *clear_color =
	    d->alpha_blend_requested ? &background_color_transparent : &background_color_idle;
                      ...
                      const struct comp_render_dispatch_data *d)
{
	...
	for (uint32_t i = 0; i < d->target.view_count; i++) {
		...
		ret = render_gfx_mesh_alloc_and_write( //
		    ...
		    d->passthrough_camera.valid ? d->passthrough_camera.sampler : md->views[i].src_sampler,
		    d->passthrough_camera.valid ? d->passthrough_camera.image_view : md->views[i].src_image_view,
	...
	render_gfx_begin_target(       //
	    render,                    //
	    d->target.gfx.rtr,         //
	    d->alpha_blend_requested ? &background_color_transparent : &background_color_active); //
                       ...
                       VkImageLayout transition_to)
{
	...
	const VkClearColorValue *color = NULL;
	if (d->alpha_blend_requested) {
		color = &background_color_transparent;
	} else {
		color = layer_count == 0 ? &background_color_idle : &background_color_active;
	}
