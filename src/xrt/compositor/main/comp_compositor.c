...
compositor_layer_commit(struct xrt_compositor *xc, xrt_graphics_sync_handle_t sync_handle)
{
	...
	c->base.frame_params.alpha_blend_requested =
	    c->base.layer_accum.data.env_blend_mode == XRT_BLEND_MODE_ALPHA_BLEND;
                                   ...
                                   struct xrt_system_compositor **out_xsysc)
{
	...
	bool dropped_alpha_blend_for_compute = false;
	sys_info->supported_blend_mode_count = 0;
	for (size_t i = 0; i < xdev->hmd->blend_mode_count; ++i) {
		enum xrt_blend_mode blend_mode = xdev->hmd->blend_modes[i];
		assert(u_verify_blend_mode_valid(blend_mode));
		if (c->settings.use_compute && blend_mode == XRT_BLEND_MODE_ALPHA_BLEND) {
			dropped_alpha_blend_for_compute = true;
			continue;
		}

		sys_info->supported_blend_modes[sys_info->supported_blend_mode_count++] = blend_mode;
	}
	if (sys_info->supported_blend_mode_count == 0) {
		sys_info->supported_blend_modes[0] = XRT_BLEND_MODE_OPAQUE;
		sys_info->supported_blend_mode_count = 1;
	}
	if (dropped_alpha_blend_for_compute) {
		COMP_WARN(c, "Compute compositor path does not support alpha blend passthrough, filtering it out.");
	}
